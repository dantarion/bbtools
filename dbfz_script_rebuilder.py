...
import dbfz_script_parser

commandDB  = json.loads(open("static_db/dbfz/commandDB.json").read())
# characters = json.loads(open("static_db/dbfz/characters.json").read())
# moveInputs = json.loads(open("static_db/dbfz/named_values/move_inputs.json").read())

commandDBLookup = {}
# namedValueLookup = {}

dbfz_script_parser.commandDB = commandDB
# dbfz_script_parser.characters = characters
...
uponLookup = {v: k for k, v in dbfz_script_parser.uponLookup.items()}
# slotLookup = {v: k for k, v in dbfz_script_parser.slotLookup.items()}
...
class Rebuilder(astor.ExplicitNodeVisitor):
    def visit_Module(self,node):
        ...
        for function in node.body:
            ...
            if function.name == "CmnActFuttobiBGTrans" and not futtobiBGTransVisited:
                futtobiBGTransVisited = True
            elif function.name == "CmnActFuttobiBGTrans":
                continue
            if function.name == "CmnActLose" and not cmnActLoseVisited:
                cmnActLoseVisited = True
            elif function.name == "CmnActLose":
                continue
    ...
    def visit_RootFunctionDef(self,node):
        ...
        if len(node.decorator_list) == 1:
            ...
            if node.decorator_list[0].id == "State" and hasattr(node, '_index'):
            ...
            elif node.decorator_list[0].id == "State":
                output.seek(0,2)
                writeCommandByName("startState",[node.name])
                self.visit_body(node.body)
                writeCommandByName("endState",[])
...
if len(sys.argv) < 3:
    print "Usage:python dbfz_script_rebuilder.py inputfile.py outputfile.dbzscript"
    sys.exit(1)
...
rebuild_dbzfscript(sourcefn, outfn)
